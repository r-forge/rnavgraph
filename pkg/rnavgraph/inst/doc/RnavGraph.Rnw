...
biocLite("RDRToolbox")
@ 
\texttt{RnavGraph} and \texttt{RnavGraphImageData} are then installed as follows
<<eval=FALSE>>=
install.packages("RnavGraph")
install.packages("RnavGraphImageData")
@ 

In order to install \texttt{rggobi} and \texttt{Rgraphviz} you need to make sure to install the program \texttt{graphviz} and \texttt{ggobi} before the corresponding \texttt{R} packages.
