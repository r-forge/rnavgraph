...
\texttt{RnavGraph} requires \texttt{tcl/tk} version 8.5. Any other version won't work with \texttt{RnavGraph}. You may check your \texttt{tcl/tk} version accessible from \texttt{R} by starting \texttt{R} and entering
<<eval = FALSE>>=
library(tcltk)
.Tcl('set tcl_version')
@ 
...
install.packages(c("PairViz", "scagnostics", "rgl", "grid",
 "MASS", "RGtk2", "hexbin", "vegan", "png"), dependencies = TRUE)
@ 
...
In order to install \texttt{rggobi} and \texttt{Rgraphviz} you need to make sure to install the programs \texttt{graphviz} and \texttt{ggobi} before installing the corresponding \texttt{R} packages.\\
...
\begin{figure}[h]
  ...
  \caption{Mapping of the \texttt{group} argument to colors in the ggobi display.}  
\end{figure}
...
aloi_images[1:5]
@ 
We sample 400 images, so that we do not have to deal with all 2250 images at once.
<<eval=FALSE>>=
...
@ 
or if you only pass on the \texttt{NG\_data} object, you may use
<<eval=FALSE>>=
...
@ 
The default is the \texttt{tk2d} display.
