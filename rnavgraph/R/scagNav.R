...
# Fraction of top values to use in (0,1]
	...
	iScags <- match(scagNames,rownames(scag))
	
	scagColNames <- colnames(scag)
...
#ggobi glyph_size 
	...
	if(require(scagnostics)){
		...
		for(i in seq(1,2*length(graphList),by = 2)) {
			...
			ng.graphList[[i]] <- ng_graph(name = paste("3d", names(graphList)[j]), graph = LG, sep = sep,layout = layout)
			ng.graphList[[i+1]] <- ng_graph(name = paste("4d", names(graphList)[j]), graph = complement(LG), sep = sep,layout = layout)
			ng.vizList[[i]] <- ng_2d(data, ng.graphList[[i]], glyphs = glyphs, images = images)
			ng.vizList[[i+1]] <-  ng_2d(data, ng.graphList[[i+1]], glyphs = glyphs, images = images)
		}
